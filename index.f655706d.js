...
function handleUserGuess(event) {
    ...
    let id = "";
    userGuess = "";
    if (randomNumber === guess) id = "success";
    else if (randomNumber < guess) id = "less";
    else id = "greater";
...
function getLedType(key, className, digit) {
    if (ledLight[key][className].includes(digit)) {
        if (isReloadAllowed) return displayInfo.id;
        return "active";
    }
    return "disabled";
}
function addLedDisplay() {
    ...
    for (const digit of digits){
        ...
        Object.keys(ledLight).forEach((key)=>{
            ...
            if (key !== "middle") Object.keys(ledLight[key]).forEach((secKey)=>{
...
//# sourceMappingURL=index.f655706d.js.map
